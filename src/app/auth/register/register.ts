...
export function gcEmailValidator(control: AbstractControl): ValidationErrors | null {
  ...
  const userTypeControl = control.get('userType');

  if (!emailControl || !userTypeControl || !emailControl.value) {
    return null;
  }

  const email: string = emailControl.value;
  const userType: string = userTypeControl.value;
  ...
  if ((userType === 'student' && domainPart !== 'gordoncollege.edu.ph') || (userType === 'admin' && domainPart !== 'gcadmin.edu.ph')) {
